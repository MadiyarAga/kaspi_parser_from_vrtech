...
#     """Получаем дополнительные данные о товаре (характеристики, изображения)"""
#
#     url = f"https://kaspi.kz/yml/product-view/product/{product_id}"
#
#     headers = {
#         "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
#                       "AppleWebKit/537.36 (KHTML, like Gecko) "
...
def save_product_to_db(product_data):
    """Сохраняем товар в БД, если его ещё нет"""
    ...
    if existing:
        ...
        # Обновляем данные при повторном парсе
        cursor.execute("""
                UPDATE products
                SET category = %s,
                    min_price = %s,
        ...
        conn.commit()
        print(f"\nДанные о товаре '{product_data['name']}' обновлены (ID={product_id}).")
    else:
        cursor.execute("""
        ...
        conn.commit()
        print(f"\nТовар '{product_data['name']}' добавлен в БД (ID={product_id}).")
...
def save_offers_to_db(offers, product_id):
    ...
    cursor.close()
    conn.close()
    print(f"\n{len(offers)} офферов сохранены в БД для product_id={product_id}.")
